...
describe("getStatusMessage", () => {
  test("preparing の場合は '議案の上程前' を返す", () => {
    expect(getStatusMessage("preparing", null)).toBe("議案の上程前");
  });

  test("preparing の場合は statusNote があっても '議案の上程前' を返す", () => {
    expect(getStatusMessage("preparing", "審議中メモ")).toBe("議案の上程前");
  });

  test("preparing 以外で statusNote がある場合はそれを返す", () => {
    expect(getStatusMessage("introduced", "総務文教委員会に付託")).toBe(
      "総務文教委員会に付託"
    );
  });
