...
describe("highlightMatch", () => {
  it("一致箇所の前後で分ける", () => {
    expect(highlightMatch("ガソリン税を安くする議案", "税")).toEqual([
      { text: "ガソリン", matched: false },
      { text: "税", matched: true },
      { text: "を安くする議案", matched: false },
    ]);
  });

  it("先頭が一致したら前の断片を作らない", () => {
    expect(highlightMatch("ガソリン税の議案", "ガソリン")).toEqual([
      { text: "ガソリン", matched: true },
      { text: "税の議案", matched: false },
    ]);
  });

  it("末尾が一致したら後の断片を作らない", () => {
    expect(highlightMatch("ガソリン税の議案", "議案")).toEqual([
  ...
  it("見つからなければ分けない", () => {
    expect(highlightMatch("ガソリン税の議案", "暮らし")).toEqual([
      { text: "ガソリン税の議案", matched: false },
    ]);
  });

  it("空クエリなら分けない", () => {
    expect(highlightMatch("ガソリン税の議案", "   ")).toEqual([
      { text: "ガソリン税の議案", matched: false },
    ]);
  });
