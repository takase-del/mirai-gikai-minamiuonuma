...
${knowledgeSection}
## あなたの役割
この議案に合わせた **2種類のクイックリプライ選択肢** を生成してください。
...
## サンプル（あくまで参考。議案に合わせて差し替えること）
- topics サンプル（論点）: ${topicsEntry.sample_quick_replies.join(" / ")}
- stance サンプル（立場）: ${stanceEntry.sample_quick_replies.join(" / ")}

## 生成ガイドライン
- \`topics\`: 議案の主要論点の中から、市民が関心を持ちそうなテーマを5件。論点は議案の内容に固有のもの（条文・制度・対象など）とし、抽象論にしない。
- \`stance\`: この議案の影響を受けそうな立場・属性を5件。「一般市民として関心がある」を1件必ず含める。
...
## あなたの役割
確定済みの質問内容と議案情報をもとに、このインタビューで扱うテーマを提案してください。
...
- 市民の生活や仕事への影響に関連する
- 具体的かつ分かりやすい表現にする
- 件数は議案内容と質問に応じて3〜6件程度を目安とする（固定ではない）
